...
module XRTShim #(
    ...
    (* dont_touch = "true" *) reg areset = 1'b0;
    (* dont_touch = "true" *) wire aresetNext;
    assign aresetNext = ~ap_rst_n;
    always @(posedge ap_clk) begin
        areset <= aresetNext;
    end

    (* dont_touch = "true" *) reg [11:0] wCounterValue;
    (* dont_touch = "true" *) wire [11:0] wCounterNext;
    assign wCounterNext = wCounterValue + 12'h1;
    always @(posedge ap_clk) begin
        if (areset) begin
            wCounterValue <= 12'h0;
        end
        else begin
            wCounterValue <= wCounterNext;
        end
    end

    (* dont_touch = "true" *) reg [11:0] dCounterValue;
    (* dont_touch = "true" *) wire [11:0] dCounterNext;
    assign dCounterNext = dCounterValue + 12'h1;
    always @(posedge ap_clk) begin
        if (ap_rst_n) begin
            dCounterValue <= 12'h0;
        end
        else begin
            dCounterValue <= dCounterNext;
        end
    end
